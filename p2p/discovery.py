"""Peer discovery using UDP broadcast for local network - DEBUG VERSION"""

import os
import socket
import json
import threading
import time
from typing import Callable, Set


VERBOSE = os.getenv("DC_VERBOSE") == "1"


def debug(msg: str):
    if VERBOSE:
        print(msg)


class PeerDiscovery:
    """Peer discovery using UDP broadcast with improved error handling"""
    
    BROADCAST_PORT = 37020
    DISCOVERY_INTERVAL = 5
    
    def __init__(self, peer_id: str, p2p_port: int, on_peer_found: Callable, transport=None):
        self.peer_id = peer_id
        self.p2p_port = p2p_port
        self.on_peer_found = on_peer_found
        self.transport = transport
        self.running = False
        self.discovered_peers: Set[str] = set()
        
        self.broadcast_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
        self.broadcast_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        
        self.listen_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.listen_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        
        self.rendezvous = ""
        self.actual_port = self.BROADCAST_PORT
    
    def start(self, rendezvous: str):
        """Start peer discovery with fallback port binding"""
        self.rendezvous = rendezvous
        self.running = True
        
        print(f"Peer discovery started for room '{rendezvous}'")
        
        # Improved port binding with fallback
        port_bound = False
        for attempt_port in range(self.BROADCAST_PORT, self.BROADCAST_PORT + 10):
            try:
                self.listen_socket.bind(('', attempt_port))
                self.actual_port = attempt_port
                port_bound = True
                
                debug(f"Bound to UDP port {attempt_port}")
                
                if attempt_port != self.BROADCAST_PORT:
                    print(f"⚠️  Port {self.BROADCAST_PORT} in use, using port {attempt_port}")
                break
                
            except OSError as e:
                debug(f"Port {attempt_port} failed: {e}")
                continue
        
        if not port_bound:
            print(f"❌ Could not bind discovery port (tried {self.BROADCAST_PORT}-{self.BROADCAST_PORT + 9})")
            print(f"⚠️  Peer discovery will be limited - you can still connect manually")
        
        # If a transport is provided and supports register_handler/send, use it.
        if self.transport:
            self.transport.register_handler(self._on_transport_message)
            # Start a lightweight broadcaster thread that uses transport.send
            broadcast_thread = threading.Thread(
                target=self._broadcast_presence_via_transport,
                daemon=True
            )
            broadcast_thread.start()
            debug("Transport-based broadcast thread started")
        else:
            broadcast_thread = threading.Thread(
                target=self._broadcast_presence,
                daemon=True
            )
            broadcast_thread.start()
            debug("Broadcast thread started")

        if port_bound and not self.transport:
            listen_thread = threading.Thread(
                target=self._listen_for_peers,
                daemon=True
            )
            listen_thread.start()
            debug("Listen thread started")
        
        print("✓ Peer discovery ready")
    
    def _broadcast_presence(self):
        """Periodically broadcast presence"""
        broadcast_count = 0
        while self.running:
            try:
                announcement = {
                    'type': 'peer_announcement',
                    'peer_id': self.peer_id,
                    'p2p_port': self.p2p_port,
                    'rendezvous': self.rendezvous
                }
                
                message = json.dumps(announcement).encode('utf-8')
                broadcast_count += 1
                # Broadcast to standard port and actual port if different
                ports_to_try = {self.BROADCAST_PORT, self.actual_port}
                
                for port in ports_to_try:
                    try:
                        self.broadcast_socket.sendto(
                            message,
                            ('255.255.255.255', port)
                        )
                    except Exception as e:
                        debug(f"Broadcast to port {port} failed: {e}")
                        
            except Exception as e:
                if self.running:
                    print(f"⚠️  Broadcast error: {e}")
            
            time.sleep(self.DISCOVERY_INTERVAL)
    
    def _listen_for_peers(self):
        """Listen for peer announcements"""
        self.listen_socket.settimeout(1.0)
        
        debug(f"Listening for peers on port {self.actual_port}")
        
        received_count = 0
        while self.running:
            try:
                data, addr = self.listen_socket.recvfrom(1024)
                received_count += 1
                
                debug(f"Received UDP packet #{received_count} from {addr}")
                
                try:
                    message = json.loads(data.decode('utf-8'))
                    debug(f"Parsed announcement: type={message.get('type')}, peer_id={message.get('peer_id')}, rendezvous={message.get('rendezvous')}")
                except json.JSONDecodeError as e:
                    debug(f"Invalid JSON in packet: {e}")
                    continue
                
                # Skip our own announcements
                if message.get('peer_id') == self.peer_id:
                    debug("Ignoring our own announcement")
                    continue
                
                # Filter by room (rendezvous point)
                msg_rendezvous = message.get('rendezvous')
                if msg_rendezvous != self.rendezvous:
                    debug(f"Wrong room - got '{msg_rendezvous}', need '{self.rendezvous}'")
                    continue
                
                peer_id = message.get('peer_id')
                peer_port = message.get('p2p_port')
                peer_ip = addr[0]
                
                # Validate data
                if not peer_id or not peer_port:
                    debug("Missing peer_id or peer_port")
                    continue
                
                if peer_id:
                    is_new = peer_id not in self.discovered_peers
                    if is_new:
                        self.discovered_peers.add(peer_id)
                        print(f"✓ Discovered peer: {peer_id}")
                    debug(f"Peer details - IP: {peer_ip}, Port: {peer_port}")
                    
                    if self.on_peer_found:
                        try:
                            debug("Calling on_peer_found callback")
                            self.on_peer_found(peer_id, peer_ip, peer_port)
                            debug("Callback completed")
                        except Exception as e:
                            print(f"⚠️  Error in peer discovery callback: {e}")
                            import traceback
                            traceback.print_exc()
                            
            except socket.timeout:
                continue
            except Exception as e:
                if self.running:
                    print(f"⚠️  Listen error: {e}")
                    import traceback
                    traceback.print_exc()
    
    def get_discovered_peers(self) -> Set[str]:
        """Get set of discovered peer IDs"""
        return self.discovered_peers.copy()

    # Transport-based helpers
    def _broadcast_presence_via_transport(self):
        while self.running:
            try:
                announcement = {
                    'type': 'peer_announcement',
                    'peer_id': self.peer_id,
                    'p2p_port': self.p2p_port,
                    'rendezvous': self.rendezvous
                }
                # broadcast on same port(s)
                ports_to_try = {self.BROADCAST_PORT, self.actual_port}
                for port in ports_to_try:
                    try:
                        self.transport.send(('255.255.255.255', port), announcement)
                    except Exception as e:
                        debug(f"Transport broadcast to port {port} failed: {e}")
            except Exception as e:
                if self.running:
                    print(f"⚠️  Broadcast error: {e}")
            time.sleep(self.DISCOVERY_INTERVAL)

    def _on_transport_message(self, message, addr):
        # Expect JSON-like dict from transport
        try:
            msg = message
            if msg.get('peer_id') == self.peer_id:
                return
            if msg.get('rendezvous') != self.rendezvous:
                return
            peer_id = msg.get('peer_id')
            peer_port = msg.get('p2p_port')
            peer_ip = addr[0]
            if peer_id:
                is_new = peer_id not in self.discovered_peers
                if is_new:
                    self.discovered_peers.add(peer_id)
                    print(f"✓ Discovered peer: {peer_id}")
                if self.on_peer_found:
                    try:
                        self.on_peer_found(peer_id, peer_ip, peer_port)
                    except Exception:
                        pass
        except Exception as e:
            debug(f"Transport message handling error: {e}")
    
    def stop(self):
        """Stop peer discovery"""
        print(f"🔧 DEBUG: Stopping peer discovery")
        self.running = False
        try:
            self.broadcast_socket.close()
        except:
            pass
        try:
            self.listen_socket.close()
        except:
            pass


def init_mdns(peer_id: str, p2p_port: int, rendezvous: str, 
              on_peer_found: Callable) -> PeerDiscovery:
    """Initialize peer discovery service"""
    discovery = PeerDiscovery(peer_id, p2p_port, on_peer_found)
    discovery.start(rendezvous)
    return discovery